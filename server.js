...
app.post('/decode', async (req, res) => {
  try {
    const {
      ...
      // other fields are accepted and may be used by the decoder, whatsappTypeMessageToDecode } = req.body || {};

    if (!url || !mediaKey) {
      return res.status(400).json({ error: 'url and mediaKey are required' });
    }

    const buffer = await decodeWhatsAppMedia({ url, mediaKey, messageType, mimetype, whatsappTypeMessageToDecode: _whatsType });
